...
[ -f "$MODPATH/libpatch.txt" ] && LIBPATCH=$(cat $MODPATH/libpatch.txt)
...
if [ -d "/odm/etc/" ] && [ -f "/data/adb/modules/ViPER4Android-RE/odm/etc/audio_effects.xml" ]; then
    ui_print "Binding audio_effects.xml to odm partition..."
