...
# Copy driver files from common/files
ui_print "- Copying driver files..."
if [ -d "$MODPATH/common/files" ]; then
  cp_ch -n $MODPATH/common/files/libv4a_re_$ABI32.so $MODPATH$LIBDIR/lib/soundfx/libv4a_re.so 0644
  if [ "$IS64BIT" ]; then
    cp_ch -n $MODPATH/common/files/libv4a_re_$ABI.so $MODPATH$LIBDIR/lib64/soundfx/libv4a_re.so 0644
  fi
  ui_print "  Drivers copied successfully"
else
  ui_print "  Warning: common/files directory not found"
fi
...
# Process scripts and set permissions
for i in $(find $MODPATH -type f -name "*.sh" -o -name "*.prop" -o -name "*.rule" 2>/dev/null); do
  [ -f $i ] && { 
    sed -i -e "/^#/d" -e "/^ *$/d" $i
    [ "$(tail -1 $i)" ] && echo "" >> $i
  } || continue
...
# Cleanup for 32-bit devices
$IS64BIT || for i in $(find $MODPATH/system -type d -name "lib64" 2>/dev/null); do rm -rf $i; done
...
if [ -d $MODPATH/system/vendor ]; then
  ...
  for FILE in $(find $MODPATH/system/vendor -type f -name "*.apk" 2>/dev/null); do
